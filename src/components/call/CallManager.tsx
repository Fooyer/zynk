...
export function CallManager() {
  const {
    status, peerId, peerUsername, volume, mode,
    setActive, setMuted, setScreenSharing, setSharingAudio, reset,
  } = useCallStore();
  ...
  useEffect(() => {
    ...
    const onScreenShareToggle = async () => {
      ...
      // Sem Electron não existe IPC pra listar sources com thumbnail — o
      // WebView2 (Chromium) já abre o seletor nativo do Windows sozinho
      // assim que getDisplayMedia roda, então pula direto pra captura.
    ...
    // getDisplayMedia já abre o seletor nativo do Windows — não precisa de
    // um source escolhido antes (isso só existia pro IPC do Electron).
    startScreenShareRef.current = async () => {
      const pc = pcRef.current;
      if (!pc) return;
